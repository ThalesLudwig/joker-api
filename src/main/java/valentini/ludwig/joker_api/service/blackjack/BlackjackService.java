package valentini.ludwig.joker_api.service.blackjack;
...
public class BlackjackService {
    ...
    public static GameResponse setBet(double reqBet) {
        ...
        if (reqBet > cash) {
            commands = new String[]{ "bet", "cash", "stop" };
            ...
            commands = new String[]{ "bet", "cash", "hand", "stop" };
            bet = reqBet;
            cash = cash - bet;
    ...
    private static GameResponse win(String message, double cashEarned) {
        String[] commands = new String[]{ "bet", "cash", "stop" };
    ...
    public static GameResponse hit() {
        ...
        commands = new String[]{ "hit", "cash", "hand", "stop" };
        ...
        if (isBust) {
            ...
            return win("21 - You win", 2 * bet);
        }
        return response;
    }

    public static GameResponse start(GameRequest req) {
        String[] commands = new String[]{ "hit", "cash", "hand", "stop" };
        if (deck != null) {
        ...
        if (req.bet() <= 0 || req.bet() > req.cash()) {
            commands = new String[]{ "start" };
            return new GameResponse("Bet is invalid.", req.cash(), req.bet(), 0, getHand(playerHand), getHiddenDealerHand(), commands);
        }
        setGame(req.cash(), req.bet());
        if (countCards(getHand(playerHand)) == 21) {
            return win("Natural 21 - You win", 1.5 * req.bet());
        }
